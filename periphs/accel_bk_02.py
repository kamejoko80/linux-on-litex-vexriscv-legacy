...
class AccelCore(Module, AutoCSR):
    def __init__(self, freq, baud, pads):
        ...
        # Transfer data from SoC to accel FIFO
        ...
        # SoC write data to accel FIFO behavior implementation
        ffsm.act("IDLE",
            If(fifo.writable & soc2ip_wr_edt.r,
        ...
        ffsm.act("PUSH_FIFO",
            NextValue(fifo.din, self.soc2ip_dx.storage[:8]),
            NextValue(fifo.we, 1),
        ...
        ffsm.act("PUSH_FIFO_XH",
            NextValue(fifo.din, self.soc2ip_dy.storage[:8]),
            NextValue(fifo.we, 1),
        ...
        ffsm.act("PUSH_FIFO_YH",
            NextValue(fifo.din, self.soc2ip_dz.storage[:8]),
            NextValue(fifo.we, 1),
